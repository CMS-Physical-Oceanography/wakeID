...
def get_video_start_time(video_source):
    ...
    # The filename timestamp ends in Z; subtract 3 hours for the desired EST-style output.
    return video_start_time - timedelta(hours=3)


def get_camera_time_from_frame(video_start_time, frame_index, fps):
    # Convert this frame number into seconds after the video started.
    seconds_after_start = frame_index / fps

    # Add the frame offset to the filename-derived start time.
    camera_time = video_start_time + timedelta(seconds=seconds_after_start)
...
# This stores each detected crossing event and its filename-derived camera timestamp.
crossing_events = []
crossing_count = 0
    ...
    if mode == "track":
        ...
        for track_id, detection_index in matches:
            ...
            if crossed_lr or crossed_rl:
                ...
                # Extend the boat path line to the ROI boundaries for easier visual inspection.
                ...
                camera_time = get_camera_time_from_frame(
                    video_start_time,
                    frame_index,
                    video_fps,
                )
